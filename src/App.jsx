...
import {
  ...
  AlertTriangle, ArrowUpDown, Settings, Save, Palette, Globe2, CreditCard, FileSpreadsheet, Download, CalendarDays, TicketPercent, Power, Copy, ExternalLink, RefreshCw, Share2, FileText, Boxes, ArrowDownToLine, ArrowUpFromLine, History
} from 'lucide-react'
    ...
    try {
      ...
      try {
        ...
        if (settingsResult.error) {
          ...
          const settingsRows = Array.isArray(settingsResult.data)
            ? settingsResult.data
            : []

          const settingsRow =
            settingsRows.find(item => Number(item.id) === 1) ||
            settingsRows[0] ||
            null
...
class AppErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, info) {
    console.error('Erro inesperado na BellaTen:', error, info)
  }
    ...
    if (this.state.hasError) {
      return <main className="fatal-error-page">
        <div>
          ...
          <p>Atualize a página. Se o problema continuar, entre em contato com a loja.</p>
...
function App() {
  ...
  return <AppErrorBoundary>
    <Routes>
      ...
      <Route path="/privacidade" element={<LegalPage type="privacy" settings={store.settings} />} />
      <Route path="/termos" element={<LegalPage type="terms" settings={store.settings} />} />
      <Route path="/admin/*" element={<Admin {...store} />} />
      <Route path="*" element={<NotFound settings={store.settings} />} />
    </Routes>
  </AppErrorBoundary>
}


function LegalPage({ type, settings }) {
  const storeName = settings?.store_name || DEFAULT_SETTINGS.store_name
  const storeLogo = settings?.logo_url || logo
  const isPrivacy = type === 'privacy'

  useEffect(() => {
    document.title = `${isPrivacy ? 'Política de Privacidade' : 'Termos de Uso'} | ${storeName}`
  }, [isPrivacy, storeName])

  return <main className="legal-page">
    <header className="legal-header">
      <Link to="/"><img src={storeLogo} alt={storeName} /></Link>
      <Link to="/">Voltar para a loja</Link>
    </header>

    <article className="legal-card">
      <span>{isPrivacy ? 'Privacidade' : 'Condições de uso'}</span>
      ...
      {isPrivacy ? <>
        ...
        <p>Coletamos somente os dados necessários para processar pedidos, como nome, telefone, e-mail opcional, endereço, forma de pagamento e itens selecionados.</p>
        <h2>2. Finalidade</h2>
        <p>Os dados são utilizados para registrar, preparar, entregar e acompanhar pedidos, além de prestar atendimento pelo WhatsApp.</p>
        <h2>3. Armazenamento e segurança</h2>
        <p>As informações são armazenadas em infraestrutura de banco de dados com controle de acesso. O acesso administrativo é restrito a usuários autenticados.</p>
        <h2>4. Compartilhamento</h2>
        <p>Não vendemos dados pessoais. Informações poderão ser compartilhadas apenas quando necessário para entrega, pagamento ou cumprimento de obrigação legal.</p>
        <h2>5. Direitos do titular</h2>
        <p>O cliente pode solicitar correção ou exclusão dos seus dados entrando em contato com a loja.</p>
      </> : <>
        <h2>1. Catálogo e preços</h2>
        <p>Produtos, preços, estoque e promoções podem ser alterados sem aviso prévio. A disponibilidade é confirmada no momento do pedido.</p>
        <h2>2. Pedidos</h2>
        <p>O pedido é registrado após a confirmação no checkout. A loja poderá entrar em contato para confirmar entrega, retirada ou pagamento.</p>
        <h2>3. Pagamento e entrega</h2>
        <p>Formas de pagamento e condições de entrega são informadas no checkout e podem variar conforme a região.</p>
        <h2>4. Cancelamentos</h2>
        <p>Cancelamentos e ajustes devem ser solicitados diretamente à loja. Pedidos já enviados podem ter condições específicas.</p>
        <h2>5. Uso do site</h2>
        <p>É proibido utilizar o site para fraude, tentativa de acesso indevido ou qualquer atividade ilegal.</p>
      </>}
    </article>
  </main>
}

function NotFound({ settings }) {
  const storeName = settings?.store_name || DEFAULT_SETTINGS.store_name
  const storeLogo = settings?.logo_url || logo
...
function Storefront({ products, categories, settings, loading, cart, setCart, favorites, setFavorites, reload }) {
  ...
  return <div className="site-shell" style={storefrontStyle}>
    <header className="header">
      ...
      <nav className={`nav ${menuOpen ? 'open' : ''}`}>
        <div className="container nav-inner">
          <button type="button" onClick={() => scrollToSection('inicio')}>Início</button>
          <button type="button" onClick={() => scrollToSection('produtos')}>Produtos</button>
          <button type="button" onClick={() => scrollToSection('categorias')}>Categorias</button>
          <button type="button" onClick={() => scrollToSection('sobre')}>Sobre nós</button>
          <button type="button" onClick={() => scrollToSection('contato')}>Contato</button>
          <Link to="/admin" className="mobile-only">Painel administrativo</Link>
        </div>
      </nav>
    </header>
    ...
    <main>
      ...
      <section className="brand-section" id="sobre"><div className="container brand-panel"><div className="brand-copy"><span className="eyebrow">O universo BellaTen</span><h2>Beleza que combina com você</h2><p>Produtos escolhidos para deixar sua rotina mais bonita, prática e acessível.</p><button
  type="button"
  className="primary-btn"
  onClick={() => scrollToSection('produtos')}
>
  Ver produtos
</button></div><div className="brand-highlights"><div><Sparkles /><strong>Novidades</strong><span>Achadinhos para renovar sua nécessaire.</span></div><div><Gift /><strong>Para presentear</strong><span>Monte kits lindos e especiais.</span></div><div><Heart /><strong>Com carinho</strong><span>Produtos para todos os estilos.</span></div></div></div></section>
    </main>
